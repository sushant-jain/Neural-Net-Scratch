...
def build_model(nn_hdim, num_passes=20000, print_loss=False,model={}):
    ...
    for i in xrange(1, num_passes):
        ...
        batch_count=0;
        while batch_count+batch_size<Y_train.shape[0]:
            batch_X=X_train[batch_count:batch_count+batch_size,:]
            batch_Y=Y_train[batch_count:batch_count+batch_size]
     ...
     # print "after back prop"
    	...
    	#print "z4",z4[:,:]    
            ...
            # Backpropagation
            delta5 = probs
            delta5[range(len(batch_X)), batch_Y] -= 1
...
model = build_model(500,num_passes=100, print_loss=True)
...
np.save("model_1e7_batch.npy",model)
